 ...
 * @return string
 */
function tinymce_config($cmsimple_editor, $config) {
    global $pth, $sl, $sn, $cf;

    $tiny_mode = isset($cf['editor']['tinymce_toolbar']) && file_exists($pth['folder']['plugins'] . 'tinymce/' . 'inits/init_' . $cf['editor']['tinymce_toolbar'] . '.js') ? $cf['editor']['tinymce_toolbar'] : 'full';
 ...
 *
 * @param string $element_classes  The classes of the textarea(s) that should become an editor instance. An empty array means .cmsimple-editor.
 * @param string $config  The configuration string.
 ...
 */
 function init_tinymce($classes = array(), $config = false) {
    ...
    $initClasses = 'cmsimple-editor';
    if (is_array($classes) && (bool) $classes) {
        $initClasses = implode('|', $classes);
    }

    $temp = tinymce_config($initClasses == 'cmsimple-editor', $config);

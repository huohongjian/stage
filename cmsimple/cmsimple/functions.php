...
{
    global $adm, $s, $o, $debugMode, $plugins, $errors, $cf, $bjs, $file;
...
function locator() 
{
	...
	elseif ($s > 0) {
		...
		return a(0, '') . $tx['locator']['home'] . '</a> &gt; ' . $t . str_replace('_splitToc_','',$h[$s]);
		} 
		else 
		{
...
// END pluginmanager (changed)
		...
		$changeText = $edit ? $tx['adminmenu']['normal'] : $tx['adminmenu']['edit'];
		$t .= '<li><a href="' . $sn . '?' . $su . '&' . $changeMode . '">' . $changeText . '</a></li>' . "\n";
		$t .= '<li><a href="' . $sn . '?&amp;pagemanager&amp;edit" class="">' . ucfirst($tx['adminmenu']['pagemanager']) . '</a></li>' . "\n";
		$t .= '<li><a href="' . $sn . '?&amp;normal&amp;userfiles">' . ucfirst($tx['adminmenu']['files']) . '</a>' ."\n";
		$t .= '    <ul>' . "\n";
		$t .= '    <li><a href="' . $sn . '?userfiles&subdir=' . $userfiles_path_images . '">' . ucfirst($tx['adminmenu']['images']) . '</a></li>' . "\n";
		$t .= '    <li><a href="' . $sn . '?userfiles&subdir=' . $userfiles_path_downloads . '">' . ucfirst($tx['adminmenu']['downloads']) . '</a></li>' . "\n";
		$t .= '    <li><a href="' . $sn . '?userfiles&subdir=' . $userfiles_path_media . '">' . ucfirst($tx['adminmenu']['media']) . '</a></li>' . "\n";
		$t .= '    </ul>' . "\n";
		$t .= '</li>' ."\n";
		$t .= '<li><a href="' . $sn . '?&amp;settings">' . ucfirst($tx['adminmenu']['settings']) . '</a>' ."\n"
					. '    <ul>' ."\n";
		...
		if($cf['site']['full_settings_menu'] == 'true') // CoAuthors
		{
			...
			$t .= '    <li><a href="?file=language&amp;action=array">' . ucfirst($tx['adminmenu']['language']) . '</a></li>' . "\n"
		...
		$t .= '<li id="admin_menu_logout"><a href="?&logout">' . ucfirst($tx['adminmenu']['logout']) . '</a></li>' . "\n";
		$t .= '</ul>' . "\n";
...
function languagemenu() 
{
	...
	foreach($lang_short_temp as $lang_short_element)
	{
		$lang_short[] = '|' . $lang_short_element . '|';
	}
	
	$lang_long = explode(",", stripslashes(strip_tags($cf['language']['long'])));
	
	// main installation and second languages of main installation
	if(($pth['folder']['base'] == '../' && file_exists('./cmsimplelanguage.htm')) || $pth['folder']['base'] == './')
	{ 
		$t = '';
		$r = array();
		...
		while (($p = @readdir($fd)) == true ) 
		{
			...
			{
				if (file_exists($pth['folder']['base'].$p.'/cmsimplelanguage.htm') && $p != '2lang' && $p != '2site2lang')
				{
					$r[] = '|' . $p . '|';
				}
			}
		...
		if($cf['language']['default'] != $sl)
		{
			...
			{
				$t .= '<a href="../">' . tag('img src="' . $pth['folder']['flags'] . $cf['language']['default'] . '.gif" alt="' . str_replace('|','',str_replace($lang_short,$lang_long,'|' . $cf['language']['default'] . '|')).'" title="&nbsp;' . str_replace('|','',str_replace($lang_short,$lang_long,'|' . $cf['language']['default'] . '|')) . '&nbsp;" class="flag"').'</a> '; 
			}
			else
			{
				$t .= '<a href="../"><span>' . str_replace('|','',str_replace($lang_short,$lang_long,'|' . $cf['language']['default'] . '|')) . '</span></a> ';
			}
		}
		$v = count($r); for($i = 0;
		$i < $v;
		$i++) 
		{
			if ($sl != str_replace('|','',$r[$i])) 
			{
				...
				{
					$t.= '<a href="' . $pth['folder']['base'] . str_replace('|','',$r[$i]).'/">' . tag('img src="' . $pth['folder']['flags'] . str_replace('|','',$r[$i]) . '.gif" alt="' . str_replace('|','',str_replace($lang_short,$lang_long,$r[$i])) . '" title="&nbsp;' . str_replace('|','',str_replace($lang_short,$lang_long,$r[$i])) . '&nbsp;" class="flag"') . '</a> ';
				} 
				else 
				{
					$t .= '<a href="' . $pth['folder']['base'] . str_replace('|','',$r[$i]).'/"><span>' . str_replace('|','',str_replace($lang_short,$lang_long,$r[$i])) . '</span></a> ';
				}
			}
		}
		return ''.$t.'';
	}
	// END main installation and second languages of main installation
	
	
	// second languages of subsites
	if($pth['folder']['base'] == '../../' && file_exists('./cmsimplelanguage.htm')) 
	{ 
		$t = '';
		$r = array();
		$fd = @opendir('../');
		while (($p = @readdir($fd)) == true ) 
		{
			if (@is_dir('../'.$p)) 
			{
				if (file_exists('../'.$p.'/cmsimplelanguage.htm'))
				{
					$r[] = '|' . $p . '|';
				}
			}
		...
		$i++) 
		{
			if ($sl != str_replace('|','',$r[$i])) 
			{
				if (is_file($pth['folder']['flags'].'/' . str_replace('|','',$r[$i]) . '.gif')) 
				{
					$t.= '<a href="../' . str_replace('|','',$r[$i]) . '/">' . tag('img src="' . $pth['folder']['flags'] . str_replace('|','',$r[$i]) . '.gif" alt="' . str_replace('|','',str_replace($lang_short,$lang_long,$r[$i])) . '" title="&nbsp;' . str_replace('|','',str_replace($lang_short,$lang_long,$r[$i])) . '&nbsp;" class="flag"') . '</a> ';
				} 
				else 
				{
					$t .= '<a href="../' . str_replace('|','',$r[$i]) . '/"><span>' . str_replace('|','',str_replace($lang_short,$lang_long,$r[$i])) . '</span></a> ';
				}
			}
		}
		return ''.$t.'';
	} 
	// END second languages of subsites
	
	
	// subsites
	if($pth['folder']['base'] == '../' && file_exists('./cmsimplesubsite.htm')) 
	{ 
		$t = '';
		$r = array();
		$fd = @opendir('./');
